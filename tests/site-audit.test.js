...
test('homepage proves the product, separates archives, surfaces games, and exposes four measurable paths', () => {
  const html = read('index.html');
  const script = read('assets/js/main.js');
  const gamesSection = (html.match(/<section id="games">([\s\S]*?)<\/section>/) || [])[1] || '';
  ...
  assert.equal((gamesSection.match(/class="app-card"/g) || []).length, 2);
  assert.match(gamesSection, /href="\/games\/chess-garden\/"/);
  assert.match(gamesSection, /href="\/games\/photo-puzzle\/"/);
